...
const profileAvatarFileInput = document.getElementById('profile-avatar-input');
...
function renderFriendsList() {
    ...
    if (e.target.classList.contains('edit-btn')) {
      ...
      li.querySelector('.edit-save-btn').addEventListener('click', async () => {
        ...
        socket.emit('edit message', { 
          messageId: id, receiverId: activeUser._id, message: newText 
        });
        li.outerHTML = '';
  ...
  messagesEl.addEventListener('click', async (e) => {
    if (e.target.classList.contains('pin-btn')) {
      const id = e.target.dataset.id;
      const updated = await apiCall(`/api/messages/${id}/pin`, { method: 'PUT' });
      if (updated.error) { 
        alert(updated.error); return; 
      }
  ...
  async function loadPinnedMessages() {
    ...
    pinnedBar.innerHTML = pinned.map(
    m => `📌 ${escapeHtml(m.message || (m.image ? 'Image' : 'Voice message'))}`).join(' &nbsp;|&nbsp; ');
  }
  ...
  async function sendVoiceMessage(audioBlob) {
    ...
    socket.emit('private message', { 
      receiverId: activeUser._id, message: '', image: '', audio: data.audioUrl 
    });
  }
  ...
  input.addEventListener('input', () => {
    if (!activeUser) return;
    socket.emit('typing', { 
      receiverId: activeUser._id, isTyping: true 
    });
    clearTimeout(typingTimeout);
    typingTimeout = setTimeout(() => {
      socket.emit('typing', { 
        receiverId: activeUser._id, isTyping: false 
      });
    }, 1000);
  });
